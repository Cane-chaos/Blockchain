...
async function main() {
    ...
    const contractOwner = await marketplace.methods.owner().call();
    ...
    try {
        ...
        try {
            const result = await marketplace.methods.getCourse(courseId).call();
            // result trả về mảng dạng [id, title, price]
            if (result[0] !== 0n && result[0] !== '0') {
                courseExists = true;
                console.log('  ℹ️  Khóa học đã tồn tại, bỏ qua bước tạo');
                console.log('  - Tên hiện tại:', result[1]);
            }
        } catch (e) {
        ...
        console.log('  - ID:', course[0]);
        console.log('  - Tên:', course[1]);
        console.log('  - Giá:', web3.utils.fromWei(course[2], 'ether'), 'ETH');
        console.log('');
        ...
        const hasPurchasedBefore = await marketplace.methods.isPurchased(buyer, courseId).call();
        ...
        const hasPurchasedAfter = await marketplace.methods.isPurchased(buyer, courseId).call();
