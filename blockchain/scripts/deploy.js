...
async function main() {
    ...
    try {
        ...
        if (!isListening) {
            throw new Error(`❌ Không thể kết nối với Ganache tại ${ganacheUrl}`);
        }
        console.log(` Đã kết nối với Ganache tại ${ganacheUrl}`);
        ...
        console.log('Đã lưu thông tin deployment vào blockchain/deployment-info.json');

        // Cấp tin nếu có biến môi trường OUTPUT_DIR (Thường dùng cho Docker deploy)
        if (process.env.OUTPUT_DIR) {
            fs.writeFileSync(
                path.join(process.env.OUTPUT_DIR, 'deployment-info.json'),
                JSON.stringify(deploymentInfo, null, 2)
            );
            console.log(`Đã lưu thông tin deployment vào ${process.env.OUTPUT_DIR}/deployment-info.json`);
        }
        ...
        if (error.message.includes('connect')) {
            console.error(`\nHãy đảm bảo Ganache đang chạy tại ${ganacheUrl}`);
        }
